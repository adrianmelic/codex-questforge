...
10. Use `questforge-visuals` for useful visual beats, not only dramatic scene art. Reuse canon visual anchors when recurring people, places, items, maps, or creatures appear again. After registering each useful static image, show it once in chat and refresh the local gallery for history. After the first visual of a local session, proactively offer the stable `#latest` visual-table link once so the player can keep it open beside the story. For 360 assets, return the viewer link instead of a static chat image and embed a well-matched approved ambience loop during initial viewer creation when it improves the scene; playback remains muted until the player enables it.
